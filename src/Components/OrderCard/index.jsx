...
const OrderCard = ({ id, image, title, price, quantity, isInSidebar }) => {
  ...
  return (
    <div className={`${ isCollapsedSidebar ? 'hidden' : 'flex'} flex-col items-center border-b p-2 border-b-slate-300 w-full`} >
        ...
        <div className='flex items-center gap-2 w-full'>
            ...
            <figure className='flex items-center justify-center w-[60px] min-w-[60px] h-[60px] overflow-hidden mx-auto rounded-lg border'>
                <img
                className='w-full h-auto rounded-lg object-cover object-top max-w-[52px] p-2'
                src={image}
                alt={title}
                />
            </figure>
            ...
            <p className='text-sm font-normal pr-2 overflow-hidden text-ellipsis line-clamp-2 max-h-[4.5em] leading-tight mb-1 text-black'>
                {title}
            </p>
            <div className='flex flex-row items-center justify-between gap-2 w-full p-1'>
                ...
                {isInSidebar && (
                <TrashIcon
                    className='h-5 w-5 text-black cursor-pointer hover:text-red-500 transition-background duration-300 ease-in-out'
